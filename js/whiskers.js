...
(function(whiskers) {
  ...
  whiskers.render = function(template, context, partials) {
    ...
    if (!whiskers.cache[template]) {
       whiskers.cache[template] = (typeof template == "function") ? template : whiskers.compile(template);
    }
  ...
  whiskers.compile = function(template) {
    ...
    // replace tags
    template = template.replace(/(\\*){(?:([\w_.]+)|>([\w_.]+)|\$([\w_.]+)|for +([\w_]+) +in +([\w_.]+)|if +(not +|)([\w_.]+)|\/(for|if))}/g, function(str, escapeChar, key, partial, ref, iterVar, forKey, ifNot, ifKey, closeStatement, offset, s) {
      if (escapeChar) return str.replace('\\\\', '');
      // {foo}
      if (key) {
        // {else}
      ...
      // {>foo}
      if (partial) return '\'+r(g(p,\''+partial+'\',true),c,p)+\'';
      // {$foo}
      if (ref) return '\'+r(g(p,\'\'+g(c,\''+ref+'\')+\'\'),c,p)+\'';
  ...
  function get(obj, key, noeval) {
    if (typeof obj == "function") return obj(key);
       
    var accessor = key.split('.');
    for (var i=0, l=accessor.length; i<l; i++) {
      obj = obj[accessor[i]];
      if (!noeval && typeof obj == "function") return obj(accessor.slice(i+1).join("."));
      if (!obj) break;
    }
    if (!obj) obj = '';
    return obj;
